...
public class ControladorInventario_PT extends SIGIPROServlet {
    ...
    private final LoteDAO lote_dao = new LoteDAO();
    ...
    protected void getAgregar_inventario(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<Lote> lotes = lote_dao.obtenerLotes();
        String lotes_str = convertirLotesAJSON(lotes);

        List<Catalogo_PT> productos = catalogo_pt_dao.obtenerCatalogos_PT();
        request.setAttribute("lotes", lotes_str);
    ...
    protected void postEditar_inventario(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            Inventario_PT pie = construirInventario(request);
            pie.setId_inventario_pt(Integer.parseInt(request.getParameter("id_inventario_pt")));
    ...
    // </editor-fold>
    private String convertirLotesAJSON(List<Lote> lotes) {
        String lotes_str = "[";

        for (Lote l : lotes) {
            lotes_str += "{\"id\":\"" + l.getNombre() + "\",\"text\":\"" + l.getNombre() + "\"}";
            lotes_str += ",";
        }

        lotes_str = lotes_str.substring(0, lotes_str.length() - 1);
        lotes_str += "]";
        return lotes_str;
    }
}
