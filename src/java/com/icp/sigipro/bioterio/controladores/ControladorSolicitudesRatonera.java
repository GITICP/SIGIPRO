...
public class ControladorSolicitudesRatonera extends SIGIPROServlet {
    ...
    private final SolicitudRatoneraDAO dao = new SolicitudRatoneraDAO();
    private final EntregaRatoneraDAO dao_en = new EntregaRatoneraDAO();
    private final CepaDAO dao_ce = new CepaDAO();
    ...
    protected void getHistorial(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        admin = verificarPermiso(203, listaPermisos);
        request.setAttribute("admin", admin);
        String redireccion = "SolicitudesRatonera/Historial.jsp";
        List<SolicitudRatonera> solicitudes_ratonera;
        request.setAttribute("pesos", pesos);
        request.setAttribute("sexos", sexos);
        if (admin) {
            try {
                solicitudes_ratonera = dao.obtenerSolicitudesRatoneraAdmCompletadas();
                request.setAttribute("listaSolicitudesRatonera", solicitudes_ratonera);
            } catch (SIGIPROException ex) {
                request.setAttribute("mensaje", helper.mensajeDeError(ex.getMessage()));
            }
            ...
            try {
                ...
                int id_usuario = (int) sesion.getAttribute("idusuario");
                Usuario u = dao_us.obtenerUsuario(id_usuario);
                solicitudes_ratonera = dao.obtenerSolicitudesRatoneraCompletadas(u.getIdSeccion());
                request.setAttribute("listaSolicitudesRatonera", solicitudes_ratonera);
            } catch (SIGIPROException ex) {
                request.setAttribute("mensaje", helper.mensajeDeError(ex.getMessage()));
            }
