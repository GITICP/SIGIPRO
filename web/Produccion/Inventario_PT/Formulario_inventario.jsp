  ...
  <div class="row">
    ...
    <div class="col-md-6">
      ...
      <div class="form-group">
        <div class="col-md-12">
          <div class="input-group">
            <select id="id_protocolo" class="select2" name="id_protocolo" required
                    oninvalid="setCustomValidity('Este campo es requerido')" style='background-color: #fff;' onchange="setCustomValidity('')">
              <c:forEach items="${protocolos}" var="protocolo">
                <c:choose>
                  <c:when test="${inventario_pt.getProtocolo().getId_protocolo() == protocolo.getId_protocolo()}" >
                    <option value=${protocolo.getId_protocolo()} selected> ${protocolo.getNombre()}</option>
                  </c:when>
                  <c:otherwise>
                    <option value=${protocolo.getId_protocolo()}> ${protocolo.getNombre()}</option>
                  </c:otherwise>
                </c:choose>
              </c:forEach>
            </select>
    ...
    <div class="col-md-6">
     ...
     <div class="col-md-6">
      ...
      <div class="form-group">
        <div class="col-md-12">
          <div class="input-group">
             <input  type="text"  id="lote" value="${inventario_pt.getLote()}" class="form-control" name="lote"  required
                    oninvalid="setCustomValidity('Este campo es requerido ')"
                    onchange="setCustomValidity('')">     
          </div>
        </div>
