  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <div class="form-group">
        <div class="col-md-12">
          <div class="input-group">
            <select id="responsable" class="select2" name="responsable" style='background-color: #fff;' required
                    oninvalid="setCustomValidity('Este campo es requerido ')"
                    onchange="setCustomValidity('')" > 
              <option value='' ></option>             
                  <c:forEach items="${usuarios}" var="usuario">              
                    <c:choose>
                      <c:when test="${usuario.getID() == analisis.getResponsable().getID()}">
                        <option value=${usuario.getID()} selected>${usuario.getNombre_completo()}</option>
                      </c:when>
                      <c:otherwise>
                        <option value=${usuario.getID()}>${usuario.getNombre_completo()}</option>
                      </c:otherwise>       
                    </c:choose>
                  </c:forEach>
            </select>
    ...
    <div class="col-md-6">
      ...
      <div class="form-group">
        <div class="col-md-12">
          <div class="input-group">
            <input  type="text" pattern="\d{1,2}/\d{1,2}/\d{4}" id="fecha-tratamiento" value="${analisis.getFecha_tratamiento_S()}" class="form-control sigiproDatePickerEspecial" name="fecha_tratamiento" data-date-format="dd/mm/yyyy" 
                    oninvalid="setCustomValidity('Este campo es requerido ')"
                    onchange="setCustomValidity('')">      
          </div>
        </div>
      ...
      <div class="form-group">
        <div class="col-md-12">
          <div class="input-group">
            <input  type="text" id="num-informe" value="${analisis.getRecetado_por()}" name="recetado_por"  class="form-control"
                    oninvalid="setCustomValidity('Este campo es requerido ')"
                    onchange="setCustomValidity('')">
          </div>
        </div>
