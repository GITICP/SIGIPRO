            ...
            <div class="widget-header">
              ...
              <div class="btn-group widget-header-toolbar">
                ...
                <c:forEach var="permiso" items="${sessionScope.listaPermisos}">
                  <c:if test="${permiso == 1 || permiso == 23}">
                    <c:set var="contienePermisoEliminar" value="true" />
                  </c:if>
                </c:forEach>
                <c:if test="${contienePermisoEliminar}">
                  <a class="btn btn-danger btn-sm boton-accion confirmable" data-texto-confirmacion="eliminar el Producto del Catálogo Externo" data-href="/SIGIPRO/Bodegas/CatalogoExterno?accion=eliminar&id_producto=${producto.getId_producto_ext()}">Eliminar</a>
                </c:if>

                <c:set var="contienePermisoEditar" value="false" />
                <c:forEach var="permiso" items="${sessionScope.listaPermisos}">
                  <c:if test="${permiso == 1 || permiso == 22}">
